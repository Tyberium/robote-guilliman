...
- Reasoning: You MUST explain your logical step-by-step breakdown under a clear "## REASONING" header. Show exactly how the rules interact.
...
Provide your step-by-step reasoning first, followed by your final ruling with exact citations. If the context is insufficient, say so explicitly."""
...
def call_llm_service(system_prompt: str, user_prompt: str) -> str:
    ...
    return (
        "## REASONING\n"
        "1. According to [Rule 01.03], the identity of the 'active player' swaps relative to whose turn is currently underway.\n"
        "2. The system dictates both players retain interactive capabilities synchronously, but sequencing priority lands on the active player.\n\n"
        "## RULING\n"
        "At any given milestone, only one player is explicitly designated as the **active player** [Rule 01.03]."
    )
...
async def whatsapp_webhook(Body: str = Form(...), From: str = Form(...)) -> Response:
    ...
    # Tier 4: Query LLM Orchestrator to get persona breakdown + reasoning
    ai_answer = call_llm_service(system_prompt=SYSTEM_PERSONA, user_prompt=user_prompt)
    
    # Tier 5: Append the transparent raw data sources directly to the WhatsApp payload
    source_footer = "\n\n--- PROVIDED SOURCE CONTEXT ---"
    for chunk in chunks:
        ref = f"Rule {chunk.rule_number}" if chunk.rule_number else "Rulebook Text"
        source_footer += f"\n• {ref} (Page {chunk.page}): \"{chunk.text[:80]}...